...
static int wm8962_suspend(struct snd_soc_codec *codec)
{
       struct wm8962_priv *wm8962 = snd_soc_codec_get_drvdata(codec);
				      ...
				      const struct i2c_device_id *id)
{
	...
	if(pdata) {
		wm8962->irq = gpio_to_irq(pdata->gpio_irq);
	}
